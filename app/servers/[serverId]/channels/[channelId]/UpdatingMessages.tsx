...
export default function UpdatingMessages(props: any) {
    ...
    function handleSubmit(formData: FormData) {
        ...
        if (message != '') {
            ...
            socket.emit('serverMessage', message, channelId, user, now)
        } else {
    ...
    socket.on('connect', () => {
        socket.emit('join-channel', String(channelId))

        socket.on('serverMessage', (message: string, user, now) => {
    ...
    return (
        <div className='pr-4'>
            <ul>
                ...
                <li className='flex mt-6' key={message.id}>
            ...
            <form action={handleSubmit} className=''>
                <input value={inputValue} onChange={(e) => setInputValue(e.target.value)} className='text-white bg-disc-anothergray w-3/4 px-2 py-3 mb-4 rounded-lg fixed bottom-0' name='newMessage' placeholder={`Message #${channelName}`}/>
